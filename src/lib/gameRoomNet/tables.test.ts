...
describe("resolveRoomInteract", () => {
  it("hits a desk from above its top edge, anywhere along its width", () => {
    ...
    expect(resolveRoomInteract(self, [], TEAMS)).toEqual({ kind: "table", tableIdx: 0 })
  })
  ...
  it("ignores a desk that is out of range", () => {
    ...
    expect(resolveRoomInteract(self, [], TEAMS)).toBeNull()
  })
  ...
  it("excludes desks with no team behind them", () => {
    const self = { x: T0.x + T0.w / 2, y: T0.y - INTERACT_PROBE_PX - 2, dir: 2 } as const
    expect(resolveRoomInteract(self, [], 0)).toBeNull()
  })
  ...
  it("lets a character in probe range beat the desk it is standing at", () => {
    ...
    expect(resolveRoomInteract(self, [seated], TEAMS)).toEqual({ kind: "probe", key: 7 })
  })
  ...
  it("gives the desk to a player facing it squarely past a wanderer on the orbit", () => {
    ...
    expect(resolveRoomInteract(self, [wanderer], TEAMS)).toEqual({ kind: "table", tableIdx: 0 })
  })
  ...
  it("still gives a teammate you are nose-to-nose with the win over the desk", () => {
    ...
    expect(resolveRoomInteract(self, [close], TEAMS)).toEqual({ kind: "probe", key: 3 })
  })
  ...
  it("treats a plant exactly like a character: close beats the desk, far does not", () => {
    ...
    expect(resolveRoomInteract(self, [nearPlant], TEAMS)).toEqual({ kind: "probe", key: 900 })
    const farPlant = { key: 900, x: px - 18, y: self.y }
    expect(resolveRoomInteract(self, [farPlant], TEAMS)).toEqual({ kind: "table", tableIdx: 0 })
  })

  it("never gives an empty desk the tie-break win either", () => {
    // 15px from the probe point: outside the tie-break radius but inside
    // interact range, so the desk takes it — unless the desk has no team.
    const self = { x: T0.x - STAND_OFF, y: T0.y + T0.h / 2, dir: 1 } as const
    const other = { key: 7, x: self.x + INTERACT_PROBE_PX - 15, y: self.y }
    expect(resolveRoomInteract(self, [other], TEAMS)).toEqual({ kind: "table", tableIdx: 0 })
    expect(resolveRoomInteract(self, [other], 0)).toEqual({ kind: "probe", key: 7 })
  })
  ...
  it("returns the nearer of two desks", () => {
    ...
    expect(resolveRoomInteract(self, [], TEAMS)).toEqual({ kind: "table", tableIdx: 1 })
  })
})
